    ...
    <div class="container-fluid p-3">
      <div class="card">
        <div class="card-header">
          <h4 class="card-title text-secondary">{{ __('employer-account.Latest Vacancies') }}</h4>
        </div>
        <div class="card-body">
                ...
                <div class="p-sm-3 p-md-0">
                  ...
                  <div class="d-flex justify-content-between py-3">
                    <div class="text-danger">
                      <i class="fas fa-clock"></i> <span class="">{{ __('employer-account.Apply Before') }}: 
                        @php    
                        $date = new DateTime($post->deadline);  
                        echo date('d', $date->getTimestamp() - time());
                        @endphp {{ __('employer-account.day[s] from now') }}</span>
                    </div>
                    <div class="text-info">
                      <i class="fas fa-eye"></i> <span>{{ __('employer-account.Views') }}: {{$post->views}}</span>
                    </div>
                  </div>
                </div>
              </div>
