        ...
        <div class="col-sm-12 col-md-12">
          <form action="{{route('post.update',['post'=>$post])}}" id="postForm" method="POST">
            ...
            <div class="form-group">
              ...
              <input type="text" placeholder="{{ __('post-create.Job title') }}" class="form-control @error('job_title') is-invalid @enderror" name="job_title" value="{{ $post->job_title }}" required autofocus >
              @error('job_title')
            ...
            <div class="form-group">
              ...
              <input type="text" placeholder="{{ __('post-create.Job location') }}" class="form-control @error('job_location') is-invalid @enderror" name="job_location" value="{{ $post->job_location }}" required >
              @error('job_location')
                  <span class="invalid-feedback" role="alert">
                      <strong>{{ $message }}</strong>
                  </span>
              ...
              <div class="row">
                ...
                <div class="col-md-6">
                  ...
                  <select name="experience" class="form-control" value="{{$post->experience}}">
                    <option value="Internship">{{ __('post-create.Internship') }}</option>
                    <option value="Entry level">{{ __('post-create.Entry level') }}</option>
                    <option value="More than 1 year">{{ __('post-create.More than') }} 1 {{ __('post-create.year') }}</option>
                    <option value="More than 2 year">{{ __('post-create.More than') }} 2 {{ __('post-create.year') }}</option>
                    <option value="More than 5+ year">{{ __('post-create.More than') }}5+ {{ __('post-create.year') }}</option>
                  </select>
                </div>
              </div>
            </div>
            ...
            <button type="button" id="postBtn" class="btn primary-btn">{{ __('post-create.Update Job listing') }}</button>
          </form>
        </div>
      </div>
    </div>
